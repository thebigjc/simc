...
void fury( player_t* p )
{
  ...
  slayer->add_action( "thunderous_roar,if=active_enemies>1" );
  ...
  slayer->add_action( "odyns_fury,if=active_enemies>1&(buff.enrage.up|talent.titanic_rage)" );
  ...
  slayer->add_action( "execute,if=variable.execute_phase&debuff.marked_for_execution.up&buff.enrage.up" );
  slayer->add_action( "bloodthirst,if=target.health.pct<35&talent.vicious_contempt&buff.brutal_finish.up&buff.enrage.up&crit_pct_current>=85|active_enemies>=6" );
