...
void devastation( player_t* p )
{
  ...
  precombat->add_action( "variable,name=trinket_priority,op=setif,value=trinket.1.is.signet_of_the_priory+2*trinket.2.is.signet_of_the_priory,if=equipped.signet_of_the_priory&variable.trinket_priority=3" );
  ...
  precombat->add_action( "variable,name=es_send_threshold,value=8", "TWW S2 Stuff" );
  ...
  st->add_action( "call_action_list,name=es,if=buff.dragonrage.up&talent.animosity&talent.engulf&set_bonus.tww2_4pc&!buff.jackpot.up&variable.can_extend_dr&!cooldown.engulf.up", "Eternity Surge logic for Dragonrage as Flameshaper" );
  ...
  st->add_action( "call_action_list,name=es,if=(!talent.dragonrage|variable.next_dragonrage>variable.dr_prep_time_st|!talent.animosity|set_bonus.tww1_4pc&talent.mass_disintegrate)&(!set_bonus.tww2_4pc|!buff.jackpot.up|variable.ES_Send_threshold<=cooldown.fire_breath.remains|talent.mass_disintegrate)" );
