...
void shadow_ptr( player_t* p )
{
  ...
  filler->add_action( "call_action_list,name=heal_for_tof,if=!buff.twist_of_fate.up&buff.twist_of_fate_can_trigger_on_ally_heal.up&(talent.rhapsody|talent.divine_star|talent.halo)", "Healing spell action list for proccing Twist of Fate" );
  filler->add_action( "power_word_shield,if=!buff.twist_of_fate.up&buff.twist_of_fate_can_trigger_on_ally_heal.up&talent.crystalline_reflection", "Use PWS with CR talented to trigger TOF if there are no better alternatives available to do this as we still get insanity for a PWS cast." );
  filler->add_action( "mind_flay_insanity,target_if=max:dot.devouring_plague.remains", "Consume empowered fillers" );
  filler->add_action( "shadow_word_death,target_if=target.health.pct<(20+15*talent.deathspeaker)" );
  filler->add_action( "shadow_word_death,target_if=min:target.time_to_die,if=talent.inescapable_torment&pet.fiend.active" );
  filler->add_action( "devouring_plague,if=talent.empowered_surges&buff.surge_of_insanity.up|buff.voidform.up&talent.void_eruption" );
  filler->add_action( "halo,if=spell_targets>1", "Save up to 20s if adds are coming soon." );
  filler->add_action( "power_word_life,if=!buff.twist_of_fate.up&buff.twist_of_fate_can_trigger_on_ally_heal.up", "Using a heal with no damage kickbacks for TOF is damage neutral, so we will do it." );
  filler->add_action( "mind_flay_insanity,target_if=max:dot.devouring_plague.remains" );
  filler->add_action( "call_action_list,name=heal_for_tof,if=equipped.rashoks_molten_heart&(active_allies-(10-buff.molten_radiance.value))>=10&buff.molten_radiance.up,line_cd=5" );
  filler->add_action( "shadow_crash,if=!variable.holding_crash&talent.void_eruption&talent.perfected_form" );
  filler->add_action( "mind_flay,target_if=max:dot.devouring_plague.remains,chain=1,interrupt_immediate=1,interrupt_if=ticks>=2,interrupt_global=1" );
  filler->add_action( "divine_star" );
  ...
  main->add_action( "variable,name=pooling_mindblasts,op=set,value=0", "Are we pooling mind blasts? Currently only used for Voidweaver." );
  main->add_action( "call_action_list,name=cds,if=fight_remains<30|target.time_to_die>15&(!variable.holding_crash|active_enemies>2)" );
  ...
  main->add_action( "void_torrent,target_if=max:(dot.devouring_plague.remains*1000+target.time_to_die),if=(dot.devouring_plague.ticking|talent.void_eruption&cooldown.void_eruption.up)&talent.entropic_rift&!variable.holding_crash&(cooldown.dark_ascension.remains>=12|!talent.dark_ascension|!talent.void_blast)", "Cast Void Torrent at very high priority if Voidweaver" );
  main->add_action( "void_bolt,target_if=max:target.time_to_die,if=cooldown.void_bolt.remains<=0.1", "Use Void Bolt on the enemy with the largest time to die. Force a cooldown check here to make sure SimC doesn't wait too long (i.e. weird MF:I de-sync with GCD)" );
  main->add_action( "mind_flay_insanity,target_if=max:dot.devouring_plague.remains,if=buff.mind_flay_insanity.stack>2&talent.empowered_surges&!cooldown.void_eruption.up", "Do not overcap MFI during Empowered Surges (Archon)." );
  main->add_action( "call_action_list,name=heal_for_tof,if=!buff.twist_of_fate.up&buff.twist_of_fate_can_trigger_on_ally_heal.up&(talent.rhapsody|talent.divine_star|talent.halo)", "Hyper cringe optimizations that fish for TOF using heals. Set priest.twist_of_fate_heal_rppm=<rppm> to make this be used." );
  main->add_action( "devouring_plague,if=fight_remains<=duration+4", "Spend your Insanity on Devouring Plague at will if the fight will end in less than 10s" );
  main->add_action( "devouring_plague,target_if=max:target.time_to_die*(dot.devouring_plague.remains<=gcd.max|variable.dr_force_prio|!talent.distorted_reality&variable.me_force_prio),if=insanity.deficit<=35&talent.distorted_reality|buff.mind_devourer.up&cooldown.mind_blast.up&(cooldown.void_eruption.remains>=3*gcd.max|!talent.void_eruption)&talent.mind_devourer|buff.entropic_rift.up", "Use Devouring Plague to maximize uptime. Short circuit if you are capping on Insanity within 35 With Distorted Reality can maintain more than one at a time in multi-target." );
  main->add_action( "void_torrent,target_if=max:(dot.devouring_plague.remains*1000+target.time_to_die),if=!variable.holding_crash&!talent.entropic_rift&cooldown.mind_blast.full_recharge_time>=2,target_if=dot.devouring_plague.remains>=2.5", "Use Void Torrent if it will get near full Mastery Value and you have Cthun and Void Eruption. Prune this action for Entropic Rift Builds." );
